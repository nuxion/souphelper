...
class SoupHelper:
    ...
    def getBlockByID(self, strTag, l):
        """ A diferencia de getAllBlocks, puedo es un wrapper de
        find_all para setear el limite de la cantidad de resultados.
        setea self.block con un objeto del tipo Soup. """
        result = self.soupbase.find_all(id=re.compile(strTag), limit=l)
        print (type(result))
        print (len(result))
        self.block = result[0]
    ...
    def linksInBlocks(self, strTag):
        ...
        for b in self.allBlocks:
            ...
            print (text)
            print ("----")
    def linksInElements(self, strTag):
        """ A diferencia de linksInBlocks, extrae los links cuando ya esta en
        <a href="http://> y no necesariamente en un bloque.
        A futuro deberia dejarse un solo metodo. """
        self.dictLinks={}
        for b in self.allBlocks:
